...
- forkする: 再現可能なPRとして新しい世界線を追加する

## コアループ

1. 2036年に破滅する基準世界を観察する。
2. フィクション・レンズを一つ選ぶ。
3. 一つの機能を現実の介入へ翻訳する。
4. 技術ツリー、費用、副作用、失敗条件を組む。
5. 介入あり・なしを同じ条件で実行する。
6. 少なくとも一つのノードを遅らせ、間に合わない条件を試す。
7. 結果と限界をPRにし、比較できる世界を増やす。
...
将来のWeb MVPは次の6画面に限定する。表示層はルールエンジンの状態を所有しない。

1. 世界の入口: 日本2036と破滅条件を30秒で理解する
2. 世界比較: 無介入と介入の差を同じ年で見る
3. 技術ツリー: 完成年、依存先、完成証拠、クリティカルパスを見る
4. 遅延実験: 一つのノードを遅らせ、破滅年の変化を見る
5. Fork Builder: 作品機能を介入JSONへ段階的に翻訳する
6. PR Preview: 変更、結果、費用、副作用、権利境界を提出前に確認する
...
7. PRを参加単位にし、議論を再現可能な差分へ着地させる。

## 成功指標

初期評価では利用者数より、次を測る。

- 初見の人が30秒で「一つのPRが一つの未来分岐」と説明できる
...
| 段階 | 到達点 | 判定 |
|---|---|---|
| 0.1 | CLI、JSON契約、日本2036、一介入、遅延比較 | 現在 |
| 0.2候補 | 読み取り専用Web比較画面と技術ツリー表示 | ルールエンジンと結果一致 |
| 0.3候補 | Fork Builderとローカル検証 | PR前にschema・権利・安全を検査 |
| 将来 | 制約内のAI説明・行動候補 | 数値更新と破滅判定を所有しない |
